(() => {
  ...
  const LOCK = "__NC_LSA_BUSINESS_DETAILS_V7__";

  if (window[LOCK]) {
    console.warn("⚠ North Carolina Business Details already running.");
    return;
  }
  ...
  const run = async () => {
    ...
    console.log(
      "🚀 NORTH CAROLINA BUSINESS DETAILS"
    );
    console.log(
      "============================================"
    );
    ...
    console.log(
      "🏁 NORTH CAROLINA BUSINESS DETAILS COMPLETE"
    );
  };
  ...
  run()
    .catch(error => {
      console.error(
        "❌ NORTH CAROLINA BUSINESS DETAILS ERROR:",
        error
      );
    })
    .finally(() => {
