(() => {
  ...
  const LOCK = "__MN_LSA_BUSINESS_DETAILS_V7__";

  if (window[LOCK]) {
    console.warn("⚠ Minnesota Business Details already running.");
    return;
  }
  ...
  const run = async () => {
    ...
    console.log(
      "🏁 MINNESOTA BUSINESS DETAILS COMPLETE"
    );
  };
  ...
  run()
    .catch(error => {
      console.error(
        "❌ MINNESOTA BUSINESS DETAILS ERROR:",
        error
      );
    })
    .finally(() => {
